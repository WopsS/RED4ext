...
void PluginSystem::Startup()
{
    ...
    for (; iter != end; iter.increment(ec))
    {
        ...
        if (entry.is_regular_file(ec) && path.extension() == L".dll")
        {
            ...
            if (m_config.blacklist.contains(stem))
            {
                spdlog::debug(L"Ignoring plugin '{}' because it is blacklisted. Path: '{}'", stem, path);
                continue;
            }
...
void PluginSystem::Load(const std::filesystem::path& aPath, bool aSearchLoadDir)
{
    ...
    if (pluginSdk < MINIMUM_SDK_VERSION || pluginSdk > LATEST_SDK_VERSION)
    {
        spdlog::warn(L"{} (version: {}) uses RED4ext.SDK v{} which is not supported by RED4ext {}. If you are the "
                     ...
                     pluginName, std::to_wstring(pluginVersion), std::to_wstring(pluginSdk), TEXT(RED4EXT_GIT_TAG),
